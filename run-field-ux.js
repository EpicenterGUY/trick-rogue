(function(root,factory){
  ...
  function fieldLoadoutModel(runState){
    ...
    const owned=state.owned.map(id=>definitionModel(id)).filter(Boolean).map(field=>({
      ...field,queued:field.id===queuedId,status:field.id===queuedId?'예약됨':'보유',actionLabel:field.id===queuedId?'예약됨':queuedId?'교체 예약':'다음 전투 예약'
    }));
...
#mapScreen.hasFieldUx #mapWrap{height:calc(58% - 78px)}
...
`;
  ...
  function renderMapFieldPanel(runtimeRoot=root){
    ...
    let panel=doc.getElementById?.('runFieldLoadoutPanel');
    if(!panel){panel=doc.createElement('section');panel.id='runFieldLoadoutPanel';panel.className='pixel';panel.dataset.m8FieldPanel='true';mapWrap.insertAdjacentElement('beforebegin',panel)}
    mapScreen.classList.add('hasFieldUx');
    ...
    const clear=panel.querySelector?.('[data-run-field-clear]');if(clear)clear.onclick=()=>clearQueuedField(runtimeRoot);
    return model;
  }
  function offerStatus(runState,fieldId){
    ...
    if(queued)return'보유 중 · 예약됨';
    if(owned)return replace?'보유 중 · 교체 예약':'보유 중 · 다음 전투 예약';
    return replace?'신규 보유 · 교체 예약':'신규 보유 · 다음 전투 예약';
  }
  function decorateEventOffer(runtimeRoot=root,node){
    ...
    const fieldId=RunFields.fieldOfferIdForNode(node,'event'),definition=RunFields.fieldDefinition(fieldId);if(!definition)return false;
    const status=offerStatus(runState,fieldId);button.dataset.runFieldOfferStatus=status;button.dataset.runFieldOfferId=fieldId;
  ...
  function decorateShopOffer(runtimeRoot=root,node){
    ...
    const fieldId=RunFields.fieldOfferIdForNode(node,'shop'),definition=RunFields.fieldDefinition(fieldId);if(!definition)return false;
    const state=RunFields.ensureRunFieldState(runState),owned=state.owned.includes(fieldId),status=offerStatus(runState,fieldId),price=owned?'재예약 무료':`${RunFields.SHOP_FIELD_COST}G`;
    button.dataset.runFieldOfferStatus=status;button.dataset.runFieldOfferId=fieldId;
  ...
  function fieldInstallationModel(state){
    const applied=state?.runFieldApplied;if(!applied?.id)return null;const definition=RunFields.fieldDefinition(applied.id)||state.field;if(!definition)return null;
    return{id:applied.id,label:definition.label||definition.name||applied.label||applied.id,description:definition.description||state.field?.description||'',source:applied.source||state.fieldSource||null};
  }
  function presentFieldInstallation(state,runtimeRoot=root){
    const model=fieldInstallationModel(state);if(!model||state.fieldInstallationPresented)return model;state.fieldInstallationPresented=true;
    const doc=runtimeRoot?.document||(typeof document!=='undefined'?document:null);if(!doc)return model;injectStyles(doc);const arena=doc.getElementById?.('arena');if(!arena)return model;
    arena.querySelector?.('.fieldInstallBanner')?.remove?.();const banner=doc.createElement('div');banner.className='fieldInstallBanner';banner.dataset.runFieldInstallation=model.id;banner.setAttribute('role','status');banner.setAttribute('aria-live','polite');
    banner.innerHTML=`<div class="kicker">플레이어 필드 설치 · ${escapeHtml(sourceLabel(model.source))}</div><b>${escapeHtml(model.label)}</b><span>${escapeHtml(model.description)}</span>`;arena.appendChild(banner);setTimeout(()=>banner.remove(),2600);return model;
  }
  ...
  function installBrowserRuntime(runtimeRoot=root){
    ...
    injectStyles(runtimeRoot.document||(typeof document!=='undefined'?document:null));wrapRenderMap(runtimeRoot);wrapRenderBattle(runtimeRoot);wrapStartBattle(runtimeRoot);wrapShowEvent(runtimeRoot);wrapShowShop(runtimeRoot);
    runtimeRoot.queueRunFieldFromMap=id=>queueOwnedField(runtimeRoot,id);runtimeRoot.clearRunFieldReservation=()=>clearQueuedField(runtimeRoot);installed=true;
    if(activeRun(runtimeRoot))renderMapFieldPanel(runtimeRoot);return true;
  }
