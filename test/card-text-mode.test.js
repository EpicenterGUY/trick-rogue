...
test('카드 면은 원문 대신 현재 compact 발동/효과 요약을 렌더한다',()=>{
  ...
  const html=CardTextMode.textCardFace(card,'hand');assert.match(html,/cardTextTrigger/);assert.match(html,/cardTextEffect/);assert.doesNotMatch(html,/아주 긴 전체 규칙 원문/);
});
...
test('손패 효과 요약은 두 줄 clamp와 한글 단어 보호를 사용한다',()=>{const source=fs.readFileSync(path.join(__dirname,'..','card-text-mode.js'),'utf8');assert.match(source,/\.cardTextEffect\{[^}]*-webkit-line-clamp:2/);assert.match(source,/\.cardTextEffect\{[^}]*word-break:keep-all/)});
