...
test('기본 규칙의 트럼프 보너스는 절대 승리권이 아니라 양쪽에 대칭적인 +3 보정이다',()=>{
...
test('기본 +3 상수와 필드가 만든 현재 전투 트럼프 보너스는 별개다',()=>{
  const state=battleState();
  EncounterRules.initializeBattle(state);
  assert.equal(BattleCore.DEFAULT_TRUMP_BONUS,3);
  assert.equal(EncounterRules.activeRulesOverride(state).trumpBonus,undefined);
  EncounterRules.setFieldFromSource(state,'outlaw_zone',{type:'event',id:'test'});
  assert.equal(EncounterRules.activeRulesOverride(state).trumpBonus,0);
  assert.equal(BattleCore.DEFAULT_TRUMP_BONUS,3,'필드 적용이 기본 상수를 바꾸면 안 된다');
});

test('역상 규칙은 트럼프 여부 분기 없이 최종 적용 숫자 비교만 뒤집는다',()=>{
  const state=battleState();
  EncounterRules.initializeBattle(state);
...
test('적 AI가 트럼프 무늬를 골라도 낮은 숫자면 높은 비트럼프에게 질 수 있다',()=>{
...
test('브라우저 호환 어댑터는 기본 +3에서 트릭 숫자 14 상한을 제거하고 최종값을 표시한다',()=>{
...
test('브라우저 리버스는 같은 무늬 여부와 무관하게 최종 적용 숫자 비교 전체를 뒤집는다',()=>{
  BattleCore.resetBrowserTrumpAdapterForTests();
  const root={
...
test('실제 판정 코드에는 한쪽 트럼프 자동 승리 분기가 남지 않고 기본값 +3만 상수로 유지된다',()=>{
