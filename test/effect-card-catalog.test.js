...
test('내부 효과 레지스트리 54장은 하나의 공용 카탈로그로 평탄화된다',()=>{
  assert.equal(Catalog.EFFECT_CARD_DEFINITIONS.length,54);assert.equal(new Set(Catalog.EFFECT_CARD_IDS).size,54);assert.equal(Cards.CARD_DEFINITIONS.length,54);
  assert.deepEqual(Cards.defaultEnabledPacks(),['all-effects']);assert.deepEqual(Object.keys(Cards.CARD_PACKS),['all-effects']);assert.equal(Cards.CARD_PACKS['all-effects'].cards.length,54);
});
test('예전 pack 선택값은 저장 호환만 하고 보상 풀을 분리하지 않는다',()=>{const all=Cards.rewardCardIds();assert.equal(all.length,54);assert.deepEqual(Cards.rewardCardIds(['pack01']),all);assert.deepEqual(Cards.rewardCardIds(['pack02']),all);assert.deepEqual(Cards.rewardCardIds(['pack03']),all);assert.deepEqual(Cards.rewardCardIds(['pack01','pack02','pack03']),all);assert.deepEqual(Cards.rewardCardIds(['pack04']),all);assert.throws(()=>Cards.rewardCardIds(['unknown-pack']),/Unknown legacy card collection reference/)});
test('내부 효과 레지스트리 54장은 표준 52장 숫자와 무늬를 사용하고 ID가 중복되지 않는다',()=>{const cards=Catalog.EFFECT_CARD_DEFINITIONS;assert.ok(cards.every(card=>['S','H','D','C'].includes(card.suit)));assert.ok(cards.every(card=>Number.isInteger(card.rank)&&card.rank>=2&&card.rank<=14));assert.equal(new Set(cards.map(card=>card.id)).size,cards.length)});
