...
test('공용 카드 인쇄 슬롯은 현재 추가 효과 카드 슬롯과 겹치지 않는다',()=>{const effectSlots=new Set(Cards.CARD_DEFINITIONS.map(card=>`${card.suit}:${card.rank}`)),collisions=Migration.PLAN.filter(entry=>effectSlots.has(`${entry.printedSuit}:${entry.printedRank}`));assert.deepEqual(collisions,[])});
