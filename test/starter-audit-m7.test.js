...
test('어느 스타터도 현재 3지역 중 한 곳에 효과 카드 4장 전부가 몰리지 않고 각 지역으로 최소 한 장의 다리를 가진다',()=>{
  const regionIds=Object.keys(CardSystemTags.REGION_REWARD_TAGS);
...
test('공통지역 카드 보상 풀은 첫 지역 선택 전 세 역할과 세 지역 연결고리를 모두 제공한다',()=>{
  ...
  for(const regionId of Object.keys(CardSystemTags.REGION_REWARD_TAGS))assert.ok(audit.regions[regionId].length>=1,regionId);
});
