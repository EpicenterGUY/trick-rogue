...
test('등록된 여섯 지역은 각각 두 내부 분기 메타데이터를 가진다',()=>{
  assert.deepEqual(Object.keys(RunStructure.REGION_BRANCHES),['region_theater','region_observatory','region_frontier','region_casino','region_red_ward','region_scrap_market']);
  ...
  assert.deepEqual(RunStructure.REGION_BRANCHES.region_red_ward.map(branch=>branch.label),['응급실','격리동']);
  assert.deepEqual(RunStructure.REGION_BRANCHES.region_scrap_market.map(branch=>branch.label),['해체장','재조립소']);
});
