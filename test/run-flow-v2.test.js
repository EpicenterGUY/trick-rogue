...
test('RUN V3 레지스트리는 공통지역·세 지역·최종 관문·최종지역을 유효한 맵으로 가진다',()=>{
...
test('지역 프로필은 정확히 3개이며 공용 60~70% / 지역 30~40% 보상 가중치를 지킨다',()=>{
  assert.equal(RunFlow.regionIds().length,3);assert.deepEqual(RunFlow.validateRegionProfiles({RunStructure}),[]);
...
test('공통지역 종료 뒤 첫 지역 선택은 세 지역을 모두 제시한다',()=>{
  const run={runFlow:RunFlow.createFlowState(),actId:'common'};const offers=RunFlow.beginRegionChoice(run,{reason:'common_complete'});assert.equal(offers.length,3);assert.deepEqual(new Set(offers),new Set(RunFlow.regionIds()));assert.equal(run.runFlow.phase,'region_choice');assert.equal(run.runFlow.choiceRound,1);
});

test('첫 지역을 고르면 STAGE 2가 되고 해당 지역 7노드 맵과 선택 이력이 남는다',()=>{
  const root=runtime(),run={runSeed:777,actId:'common',map:RunStructure.createActMap('common'),available:new Set(['c4']),completed:new Set(),actHistory:[],runFlow:RunFlow.createFlowState()};RunFlow.beginRegionChoice(run);const result=RunFlow.chooseRegion(run,'region_theater',{runtimeRoot:root});
  assert.equal(result.ok,true);assert.equal(run.runStage,2);assert.equal(run.actId,'region_theater');assert.equal(run.actName,'유랑극장');assert.equal(run.actIndex,1);assert.deepEqual(run.runFlow.visitedRegionIds,['region_theater']);assert.equal(run.map.length,7);assert.ok(run.map.every(node=>node.regionPlan?.regionId==='region_theater'));assert.ok(run.runFlow.history.some(entry=>entry.type==='region_selected'&&entry.regionId==='region_theater'));
});
...
test('내부 분기 선택은 STAGE 3/6으로 진행하며 방문 분기와 journey history를 기록한다',()=>{
  const root=runtime(),run={runSeed:500,runFlow:RunFlow.createFlowState()};RunFlow.beginRegionChoice(run);RunFlow.chooseRegion(run,'region_observatory',{runtimeRoot:root});
  const archive=run.map.find(node=>node.branchId==='archive'&&node.branchEntry),first=RunFlow.recordBranchSelection(run,archive,{runtimeRoot:root});assert.equal(first.ok,true);assert.equal(first.stage,3);assert.equal(run.runStage,3);assert.equal(run.runFlow.visitedRegionBranches[0].branchId,'archive');assert.equal(run.runFlow.journeyHistory[0].branchLabel,'기록실');
  const duplicate=RunFlow.recordBranchSelection(run,archive,{runtimeRoot:root});assert.equal(duplicate.duplicate,true);assert.equal(run.runFlow.visitedRegionBranches.length,1);
});

test('카드군이나 시작 지역은 다른 지역 선택을 막는 클래스 게이트가 아니다',()=>{
  const run={starterId:'sniper',runFlow:RunFlow.createFlowState()};RunFlow.beginRegionChoice(run);assert.deepEqual(new Set(run.runFlow.pendingRegionOfferIds),new Set(RunFlow.regionIds()));assert.equal(RunFlow.chooseRegion(run,'region_observatory',{runtimeRoot:runtime()}).ok,true);
});

test('첫 지역 보스를 끝내면 이미 방문한 지역을 제외한 다음 지역 선택으로 이어진다',()=>{
  const root=runtime(),run={runSeed:1,runFlow:RunFlow.createFlowState()};RunFlow.beginRegionChoice(run);RunFlow.chooseRegion(run,'region_frontier',{runtimeRoot:root});const boss=run.map.find(node=>node.type==='boss');run.available=new Set([boss.id]);run.currentNodeId=boss.id;RunFlow.setRunStage(run,4);const result=RunFlow.completeRegionBoss(run,boss,{runtimeRoot:root});assert.equal(result.next,'region_choice');assert.equal(run.runFlow.phase,'region_choice');assert.equal(run.runFlow.pendingRegionOfferIds.length,2);assert.ok(!run.runFlow.pendingRegionOfferIds.includes('region_frontier'));assert.deepEqual(run.runFlow.completedRegionIds,['region_frontier']);
});

test('두 번째 지역 보스 뒤에는 두 지역 흔적을 섞은 STAGE 7 최종 관문으로 전환한다',()=>{
  const root=runtime(),run={runSeed:2,runFlow:RunFlow.createFlowState()};RunFlow.beginRegionChoice(run);RunFlow.chooseRegion(run,'region_theater',{runtimeRoot:root});let branch=run.map.find(node=>node.branchEntry);RunFlow.recordBranchSelection(run,branch,{runtimeRoot:root});let boss=run.map.find(node=>node.type==='boss');run.available=new Set([boss.id]);run.currentNodeId=boss.id;RunFlow.completeRegionBoss(run,boss,{runtimeRoot:root});RunFlow.chooseRegion(run,'region_observatory',{runtimeRoot:root});branch=run.map.find(node=>node.branchEntry);RunFlow.recordBranchSelection(run,branch,{runtimeRoot:root});boss=run.map.find(node=>node.type==='boss');run.available=new Set([boss.id]);run.currentNodeId=boss.id;const result=RunFlow.completeRegionBoss(run,boss,{runtimeRoot:root});
  assert.equal(result.next,'gateway');assert.equal(run.actId,'gateway');assert.equal(run.runStage,7);assert.equal(run.runFlow.phase,'gateway');assert.deepEqual(RunFlow.gatewayPlan(run).sourceRegionIds,['region_theater','region_observatory']);assert.ok(run.map.every(node=>node.regionPlan.sourceRegionIds.length===2));assert.equal(run.runComplete,false);
});

test('최종 관문 완료 뒤 STAGE 8 최종지역으로 전환되고 최종 보스는 기존 런 종료 경로에 남는다',()=>{
  const root=runtime(),run={runSeed:3,runFlow:RunFlow.createFlowState()};run.runFlow.visitedRegionIds=['region_theater','region_frontier'];RunFlow.applyFlowAct(run,'gateway',{runtimeRoot:root,recordPrevious:false,phase:'gateway'});RunFlow.setRunStage(run,7);const result=RunFlow.completeGateway(run,{runtimeRoot:root});assert.equal(result.next,'final');assert.equal(run.actId,'final');assert.equal(run.runStage,8);assert.equal(run.runFlow.phase,'final');assert.equal(run.map.at(-1).type,'boss');assert.equal(run.runComplete,false);assert.equal(RunFlow.setRunStage(run,9),8);
});
