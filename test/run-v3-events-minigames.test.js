...
test('첫 방문 지역은 두 번째 선택에서 제외되고 남은 다섯 지역과 분기/journey history가 저장된다',()=>{
  ...
  assert.equal(run.runFlow.pendingRegionOfferIds.includes('region_frontier'),false);assert.equal(run.runFlow.pendingRegionOfferIds.length,5);assert.ok(run.runFlow.pendingRegionOfferIds.includes('region_casino'));assert.ok(run.runFlow.pendingRegionOfferIds.includes('region_red_ward'));assert.ok(run.runFlow.pendingRegionOfferIds.includes('region_scrap_market'));
});
