...
test('7.5-C 런타임은 칩 경제 뒤, 7.5-E 템포 단계보다 먼저 로드된다',()=>{const source=fs.readFileSync(path.join(__dirname,'..','enemy-behavior.js'),'utf8');assert(source.includes("loadScript('showdown-resolution.js','trick-showdown-resolution-runtime')"));assert(source.includes("if(root.ShowdownResolution){loadEncounterTempo();return;}"));assert(source.includes("if(root.ChipEconomy){loadBattleLayoutFile();return;}"));assert(source.includes("if(script?.dataset?.loaded==='true')loadEncounterTempo();else script?.addEventListener?.('load',loadEncounterTempo,{once:true});"));assert(source.includes("loadScript('encounter-tempo.js','trick-encounter-tempo-runtime')"))});
