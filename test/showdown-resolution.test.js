...
test('쇼다운 전 효과로 최종 족보가 바뀌어도 리버는 5번째 카드 선택 시점 후보 적중만 인정한다',async()=>{
  ...
  assert(events.slice(0,5).every(trigger=>trigger==='before_showdown'));assert.equal(result.riverHit.active,false,'5번째 카드 선택 당시 9는 스트레이트 후보가 아니므로 리버는 불발이다');assert.equal(result.riverHit.reason,'candidate_miss');assert.equal(result.riverHit.fifth.rank,9,'쇼다운 전 효과가 9를 6으로 바꾸기 전에 리버 적중을 잠근다');assert.equal(result.player.hand.id,'straight','쇼다운 전 효과로 최종 포커 족보 자체는 스트레이트가 될 수 있다');assert.equal(result.player.basePower,24);assert.equal(result.player.additiveTotal,10);assert.equal(result.player.preMultiplierPower,34);assert.deepEqual(result.player.multipliers.map(entry=>entry.id),['advantage']);assert.equal(result.player.multipliers[0].bonus,0.25);assert.equal(result.player.finalMultiplier,1.25);assert.equal(result.player.finalPower,43);assert.equal(result.enemy.finalPower,5);assert.equal(result.comparison.winner,'player');assert.equal(result.comparison.difference,38);assert.deepEqual(attacks.map(entry=>entry.slice(0,3)),[['player',38,'showdown']]);assert.equal(attacks[0][3].source,'showdown_player_attack');assert.equal(attacks[0][3].resolution,'power_difference');assert.equal(result.attacks.player.plannedAmount,38);assert.equal(result.attacks.player.dealt,38);assert.equal(result.attacks.enemy.plannedAmount,0);assert.equal(result.attacks.enemy.dealt,0);assert.equal(result.attackSequence.enemyAttackCancelled,true);assert.equal(battle.enemy.hp,62);assert.equal(root.run.hp,50);assert.equal(battle.chip,2,'세트가 넘어가도 칩 잔액을 리셋하지 않는다');assert.equal(battle.setIndex,2);assert.deepEqual(battle.showdownTrace,Resolution.traceLines(result));
});
