...
test('브라우저 부트스트랩은 상태→계약·금기→빌드 시너지→런 구조→경로 규칙→맵 생성→전투 레이아웃 순서를 유지한다', () => {
  ...
  assert.match(source, /function loadRunMapGeneration\(\)\{[\s\S]*?loadScript\('run-map-generation\.js','trick-run-map-generation-runtime'\)[\s\S]*?loadBattleLayout\(\)/);
});
