...
test('브라우저 부트스트랩은 상태 시스템 뒤 계약·금기를 거쳐 전투 레이아웃 보정을 연결한다', () => {
  ...
  assert.match(source, /function loadContracts\(\)\{[\s\S]*?loadScript\('contracts\.js','trick-contract-system-runtime'\)[\s\S]*?loadBattleLayout\(\)/);
