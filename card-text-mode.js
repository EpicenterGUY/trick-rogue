(function(root,factory){
  ...
  const ACTION_LABELS=Object.freeze({
    ...
    copy_previous_showdown_rank:'이전 쇼다운 숫자 복사',snapshot_set_wins:'승리 횟수 기록',showdown_power_from_memory_tiers:'기록별 쇼다운 위력',discard_selected_card:'카드 버림'
  });
  const COMPACT_TEXT=Object.freeze({
    ...
    'core.burn':{trigger:'낼 때',summary:'다른 손패 1장 버리고 칩 +1 · 카드 1장 드로우'},
    'core.plus2':{trigger:'낼 때',summary:'이번 트릭 적용 숫자 +3'},
    'core.double':{trigger:'낼 때',summary:'칩 1을 걸어 적용 숫자 +5 · 승리하면 칩 +2'}
  });
  let enabled=true;
  let installed=false;
  ...
  function compact(text,max=46){
    const normalized=String(text||'').replace(/\s+/g,' ').trim();if(normalized.length<=max)return normalized;
    const words=normalized.split(' '),kept=[];for(const word of words){if([...kept,word].join(' ').length>max)break;kept.push(word)}
    return kept.length?kept.join(' '):normalized.slice(0,max);
  }
  function summarizeEffects(card){
    if(typeof root.shortEffect==='function'){
      try{const summary=root.shortEffect(card);if(summary&&!/^효과 없음/.test(summary))return compact(summary,62)}catch(_error){}
    }
    const def=definition(card);
    if(def?.description){const cleaned=String(def.description).replace(/(?:발동|조건|효과|추가)\s*:/g,'').replace(/\s+/g,' ').trim();if(cleaned)return compact(cleaned,62)}
    const parts=[];
    for(const effect of effectList(card).slice(0,2)){
      const label=ACTION_LABELS[effect.action]||'';if(!label)continue;
      const value=Number.isFinite(effect.value)?` ${effect.value>0?'+':''}${effect.value}`:'';parts.push(`${label}${value}`);
    }
  ...
  function compactTrigger(card){
    ...
    return({on_play:'낼 때',on_trick_win:'승리',on_trick_loss:'패배',on_trick_end:'트릭 종료',on_showdown_score:'쇼다운',after_card_slotted:'슬롯 배치'})[effectList(card)[0]?.trigger]||'';
  }
  ...
  function textCardFace(card,variant='hand'){
    const suit=printedSuit(card),sym=SUIT_SYMBOL[suit]||'',rank=rankLabel(printedRank(card)),preview=buildCardCompactText(card),mini=variant==='mini',red=RED_SUITS.has(suit);
