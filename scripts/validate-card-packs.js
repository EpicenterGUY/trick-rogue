...
try{validateEnabledPacks(defaultEnabledPacks())}catch(error){errors.push(`기본 enabledPacks가 유효하지 않음: ${error.message}`)}
for (const pack of CARD_PACK_LIST) {
  ...
  if(typeof pack.id!=='string'||!/^pack\d{2}$/.test(pack.id))errors.push(`${pack.id||'(ID 없음)'}: 팩 ID는 packNN 형식이어야 함`);
  if(typeof pack.enabledByDefault!=='boolean')errors.push(`${pack.id}: enabledByDefault는 boolean이어야 함`);
  if(!Number.isInteger(pack.rewardWeight)||pack.rewardWeight<0)errors.push(`${pack.id}: rewardWeight는 0 이상의 정수여야 함`);
