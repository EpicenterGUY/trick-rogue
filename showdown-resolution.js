(function(root,factory){
  ...
  async function resolveRuntimeShowdown(runtimeRoot=defaultRoot){
    ...
    await animateBreakdown(runtimeRoot,state,model);resolveShowdownAttacks(runtimeRoot,state,runState,model);const archived=archiveBreakdown(state,model);if(runtimeRoot?.console?.debug)runtimeRoot.console.debug('[showdown 7.5-N]',archived);
  ...
  function wrapShowdown(runtimeRoot=defaultRoot){const original=runtimeRoot?.showdown;if(typeof original!=='function'||original.__tricklogShowdown75N)return false;async function wrapped(){return resolveRuntimeShowdown(runtimeRoot)}wrapped.__tricklogShowdown75N=true;wrapped.__original=original;runtimeRoot.showdown=wrapped;return true}
