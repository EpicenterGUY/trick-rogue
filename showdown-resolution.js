(function(root,factory){
  ...
  const MULTIPLIER_STAGE='7.5-N';
  const CONTENT_STAGE='7.5-P';
  const RIVER_MULTIPLIER=1.25;
  const PERFECT_SET_MULTIPLIER=1.5;
  const DEFAULT_MULTIPLIER_CAP=2.5;
  ...
  const SHOWDOWN_PHASES=Object.freeze([
    'cards_locked','pre_showdown_effects','showdown_value_changes','poker','additive_bonuses','multiplier_bonuses','final_power',
    'power_comparison','winner_determined','difference_damage','overkill','set_end'
  ]);
  ...
  function bonusFromFactor(factor){const multiplier=numeric(factor,1);if(multiplier<=0)throw new RangeError('Showdown multiplier must be greater than zero');return multiplier-1}
  function createMultiplierEntry(target,{id,label,bonus,factor,source='condition',metadata,bypassCap=false}={}){
    const added=bonus!==undefined?numeric(bonus):bonusFromFactor(factor);if(added<=-1)throw new RangeError('Showdown added multiplier must stay above -100%');if(!added)return null;
    const entry={id:id||`${source}:${target.multipliers.length+1}`,label:label||source,bonus:added,factor:1+added,source,bypassCap:bypassCap===true,pool:bypassCap===true?'cap_bypass':'general',before:null,after:null};if(metadata!==undefined)entry.metadata=metadata;return entry;
  }
  function createSide(hand,multiplierCap=DEFAULT_MULTIPLIER_CAP){return{hand:{...hand,ranks:[...(hand?.ranks||[])],suits:[...(hand?.suits||[])]},basePower:numeric(hand?.power),additives:[],additiveTotal:0,preMultiplierPower:numeric(hand?.power),multipliers:[],multiplierBonusTotal:0,generalMultiplierBonus:0,bypassMultiplierBonus:0,rawMultiplier:1,multiplierCap:numeric(multiplierCap,DEFAULT_MULTIPLIER_CAP),capApplied:false,finalMultiplier:1,multiplierProduct:1,finalPower:numeric(hand?.power)}}
  function createBreakdown({playerHand,enemyHand,setIndex=1,multiplierCap=DEFAULT_MULTIPLIER_CAP}={}){if(!playerHand||!enemyHand)throw new TypeError('Both showdown hands are required');const cap=numeric(multiplierCap,DEFAULT_MULTIPLIER_CAP);if(cap<1)throw new RangeError('Showdown multiplier cap must be at least ×1');return{stage:STAGE,attackStage:ATTACK_STAGE,riverStage:RIVER_STAGE,multiplierStage:MULTIPLIER_STAGE,contentStage:CONTENT_STAGE,multiplierCap:cap,order:[...SHOWDOWN_PHASES],setIndex,player:createSide(playerHand,cap),enemy:createSide(enemyHand,cap),riverHit:null,comparison:null,attacks:null,attackSequence:null,finalized:false}}
  ...
  function addMultiplierBonus(model,side,options={}){const target=sideOf(model,side),entry=createMultiplierEntry(target,options);if(entry)target.multipliers.push(entry);return entry}
  function addMultiplier(model,side,options={}){return addMultiplierBonus(model,side,options)}
  function applyRiverHitBonus(model,riverHit,{side='player'}={}){
    model.riverHit=riverHit?JSON.parse(JSON.stringify(riverHit)):null;
    if(riverHit?.active)addMultiplierBonus(model,side,{id:'river_hit',label:'리버 적중',bonus:bonusFromFactor(riverHit.multiplier),source:'river',metadata:{snapshotId:riverHit.snapshotId,target:riverHit.target,fifth:riverHit.fifth,candidateCount:riverHit.candidateCount}});return riverHit;
  }
  ...
  function addPerfectSetMultiplier(model,setHistory,{side='player'}={}){const perfect=detectPerfectSet(setHistory);model.perfectSet=perfect;if(perfect.active)addMultiplierBonus(model,side,{id:'perfect_set',label:'5전 전승',bonus:bonusFromFactor(perfect.multiplier),source:'trick_record',metadata:{wins:perfect.wins,results:perfect.results}});return perfect}
  function finalizeSide(side,{multiplierCap=side?.multiplierCap??DEFAULT_MULTIPLIER_CAP}={}){
    side.additiveTotal=side.additives.reduce((sum,entry)=>sum+numeric(entry.value),0);side.preMultiplierPower=Math.max(0,side.basePower+side.additiveTotal);
    const general=side.multipliers.filter(entry=>!entry.bypassCap).reduce((sum,entry)=>sum+numeric(entry.bonus),0),bypass=side.multipliers.filter(entry=>entry.bypassCap).reduce((sum,entry)=>sum+numeric(entry.bonus),0),cap=numeric(multiplierCap,DEFAULT_MULTIPLIER_CAP);
    if(cap<1)throw new RangeError('Showdown multiplier cap must be at least ×1');const rawGeneral=Math.max(0,1+general),cappedGeneral=Math.min(cap,rawGeneral),finalMultiplier=Math.max(0,cappedGeneral+bypass);
    side.generalMultiplierBonus=general;side.bypassMultiplierBonus=bypass;side.multiplierBonusTotal=general+bypass;side.rawMultiplier=Math.max(0,1+general+bypass);side.multiplierCap=cap;side.capApplied=rawGeneral>cap;side.finalMultiplier=finalMultiplier;side.multiplierProduct=finalMultiplier;side.finalPower=Math.max(0,Math.round(side.preMultiplierPower*finalMultiplier));
    for(const entry of side.multipliers){entry.before=side.preMultiplierPower;entry.after=side.finalPower}return side;
  }
  ...
  function compareFinalPower(model){
    const playerPower=Math.max(0,numeric(model?.player?.finalPower)),enemyPower=Math.max(0,numeric(model?.enemy?.finalPower)),difference=Math.abs(playerPower-enemyPower);
    const winner=playerPower>enemyPower?'player':enemyPower>playerPower?'enemy':'draw';
    return{stage:ATTACK_STAGE,playerPower,enemyPower,winner,draw:winner==='draw',difference,plannedDamage:difference,target:winner==='player'?'enemy':winner==='enemy'?'player':null};
  }
  function finalizeBreakdown(model){finalizeSide(model.player,{multiplierCap:model.multiplierCap});finalizeSide(model.enemy,{multiplierCap:model.multiplierCap});model.comparison=compareFinalPower(model);model.attacks={player:createAttack('player','enemy',model.player.finalPower),enemy:createAttack('enemy','player',model.enemy.finalPower)};model.attackSequence=null;model.finalized=true;return model}
  function percentText(value){const percent=Math.round(numeric(value)*1000)/10;return`${percent>=0?'+':''}${percent}%`}
  function multiplierText(side){if(!side.multipliers.length)return'없음';const parts=side.multipliers.map(entry=>`${entry.label} ${percentText(entry.bonus)}${entry.bypassCap?' [상한 예외]':''}`);parts.push(`합계 ×${side.finalMultiplier}`);if(side.capApplied)parts.push(`일반 상한 ×${side.multiplierCap}`);return parts.join(' · ')}
  ...
  function comparisonTraceText(model){
    const comparison=model?.comparison||compareFinalPower(model);if(comparison.draw)return'쇼다운 정산: 무승부 · 피해 없음';
    const playerWon=comparison.winner==='player',attack=playerWon?model?.attacks?.player:model?.attacks?.enemy,amount=Number.isFinite(attack?.dealt)?attack.dealt:comparison.difference;
    return`쇼다운 정산: ${playerWon?'승리':'패배'} · 차이 ${comparison.difference} · ${playerWon?'적':'플레이어'} 피해 ${amount}`;
  }
  function traceLines(model){if(!model?.finalized)finalizeBreakdown(model);const river=model.riverHit?.active?`리버: 적중 · ${model.riverHit.target?.name||'후보'} +25%`:`리버: ${model.riverHit?.reason==='candidate_miss'?'불발':'없음'}`;return[`족보: ${model.player.hand.name} ${model.player.basePower} / 적 ${model.enemy.hand.name} ${model.enemy.basePower}`,`덧셈: 나 ${additiveText(model.player)} / 적 ${additiveText(model.enemy)}`,river,`배율: 나 ${multiplierText(model.player)} / 적 ${multiplierText(model.enemy)}`,`최종 위력: ${model.player.finalPower} : ${model.enemy.finalPower}`,comparisonTraceText(model)]}
  ...
  function advantageExtra(advantage,score){return{score,advantage}}
  ...
  function recordScoreTrigger(runtimeRoot,state,model,trigger,score,advantage){
    const slots=Array.isArray(state?.slots)?state.slots:[];for(let index=0;index<slots.length;index++){
      ...
      const effectDelta=effectAfter-before;if(effectDelta)addAdditive(model,'player',{id:`${trigger}:${index}`,label:`쇼다운 효과 · ${cardLabel(slot.card,index)}`,value:effectDelta,source:'effects',metadata:{trigger,slotIndex:index}});
  ...
  function addActiveAdvantageMultipliers(runtimeRoot,state,model,advantage){const factor=numeric(advantage?.multiplier,runtimeRoot?.ShowdownAdvantage?.ADVANTAGE_MULTIPLIER||1.25),bonus=bonusFromFactor(factor);if(advantage?.playerActive)addMultiplierBonus(model,'player',{id:'advantage',label:'우세',bonus,source:advantage.playerSource||'advantage'});if(advantage?.enemyActive)addMultiplierBonus(model,'enemy',{id:'advantage',label:'우세',bonus,source:advantage.enemySource||'advantage'});return model}
  ...
  function resolveShowdownAttacks(runtimeRoot,state,runState,model){
    ...
    const comparison=model.comparison||compareFinalPower(model),playerAttack=model.attacks.player,enemyAttack=model.attacks.enemy,sequence=runtimeRoot?.document?.getElementById?.('showdownSequence');
    playerAttack.hpBefore=Math.max(0,numeric(state?.enemy?.hp));enemyAttack.hpBefore=Math.max(0,numeric(runState?.hp));playerAttack.hpAfter=playerAttack.hpBefore;enemyAttack.hpAfter=enemyAttack.hpBefore;playerAttack.dealt=0;enemyAttack.dealt=0;playerAttack.targetDefeated=false;enemyAttack.targetDefeated=false;
    if(comparison.draw){playerAttack.plannedAmount=0;enemyAttack.plannedAmount=0;playerAttack.cancelled=true;enemyAttack.cancelled=true;playerAttack.cancelReason='draw';enemyAttack.cancelReason='draw';runtimeRoot.flash?.('쇼다운 무승부 · 피해 없음')}
    else if(comparison.winner==='player'){
      playerAttack.plannedAmount=comparison.difference;enemyAttack.plannedAmount=0;enemyAttack.cancelled=true;enemyAttack.cancelReason='lost_showdown';if(comparison.difference>0)sequence?.classList?.add?.('impact');
      playerAttack.dealt=Math.max(0,numeric(runtimeRoot.damageEnemy?.(comparison.difference,'showdown',{source:'showdown_player_attack',attacker:'player',target:'enemy',resolution:'power_difference',playerPower:comparison.playerPower,enemyPower:comparison.enemyPower,difference:comparison.difference})));
      playerAttack.hpAfter=Math.max(0,numeric(state?.enemy?.hp));playerAttack.targetDefeated=playerAttack.hpAfter<=0;enemyAttack.hpAfter=enemyAttack.hpBefore;if(playerAttack.dealt>0)runtimeRoot.flash?.(`쇼다운 승리 · 적 -${playerAttack.dealt}`);
    }else{
      enemyAttack.plannedAmount=comparison.difference;playerAttack.plannedAmount=0;playerAttack.cancelled=true;playerAttack.cancelReason='lost_showdown';if(comparison.difference>0)sequence?.classList?.add?.('impact');
      enemyAttack.dealt=Math.max(0,numeric(runtimeRoot.damagePlayer?.(comparison.difference,'showdown',{source:'showdown_enemy_attack',attacker:'enemy',target:'player',resolution:'power_difference',playerPower:comparison.playerPower,enemyPower:comparison.enemyPower,difference:comparison.difference})));
      enemyAttack.hpAfter=Math.max(0,numeric(runState?.hp));enemyAttack.targetDefeated=enemyAttack.hpAfter<=0;playerAttack.hpAfter=playerAttack.hpBefore;if(enemyAttack.dealt>0)runtimeRoot.flash?.(`쇼다운 패배 · 플레이어 -${enemyAttack.dealt}`);
    }
    const enemyDefeated=playerAttack.targetDefeated,playerDefeated=enemyAttack.targetDefeated;
    model.attackSequence={stage:ATTACK_STAGE,order:['power_comparison','winner_determined','difference_damage'],winner:comparison.winner,draw:comparison.draw,difference:comparison.difference,target:comparison.target,damageDealt:comparison.winner==='player'?playerAttack.dealt:comparison.winner==='enemy'?enemyAttack.dealt:0,enemyDefeated,playerAttackCancelled:playerAttack.cancelled,enemyAttackCancelled:enemyAttack.cancelled,playerDefeated};return model.attackSequence;
  }
  ...
  async function resolveRuntimeShowdown(runtimeRoot=defaultRoot){
    ...
    const advantage=runtimeRoot.ShowdownAdvantage?.snapshot?.(state)||core.resolveShowdownAdvantage?.()||{mode:'explicit',automaticSuitComparison:false,playerActive:false,enemyActive:false,multiplier:1.25};state.advantage=advantage;runtimeRoot.renderBattle?.();
    ...
    applyRiverHitBonus(model,riverHit);addActiveAdvantageMultipliers(runtimeRoot,state,model,advantage);addPerfectSetMultiplier(model,state.setHistory);finalizeBreakdown(model);
    await animateBreakdown(runtimeRoot,state,model);resolveShowdownAttacks(runtimeRoot,state,runState,model);const archived=archiveBreakdown(state,model);if(runtimeRoot?.console?.debug)runtimeRoot.console.debug('[showdown 7.5-P]',archived);
    const playerWon=model.comparison.winner==='player',showdownDraw=model.comparison.draw;
    state.slots.forEach(slot=>runtimeRoot.runCardEffects?.('after_showdown_result',slot.card,{playerWon,draw:showdownDraw,showdownBreakdown:archived,showdownResult:archived.comparison,differenceDamage:model.comparison.difference,enemyDefeated:model.attackSequence.enemyDefeated,playerAttackCancelled:model.attackSequence.playerAttackCancelled,enemyAttackCancelled:model.attackSequence.enemyAttackCancelled,playerDefeated:model.attackSequence.playerDefeated,playerAttack:model.attacks.player,enemyAttack:model.attacks.enemy,riverHit:model.riverHit}));
  ...
  function wrapShowdown(runtimeRoot=defaultRoot){const original=runtimeRoot?.showdown;if(typeof original!=='function'||original.__tricklogShowdown75P)return false;async function wrapped(){return resolveRuntimeShowdown(runtimeRoot)}wrapped.__tricklogShowdown75P=true;wrapped.__original=original;runtimeRoot.showdown=wrapped;return true}
  ...
  return{STAGE,BURST_STAGE,ATTACK_STAGE,RIVER_STAGE,MULTIPLIER_STAGE,CONTENT_STAGE,RIVER_MULTIPLIER,PERFECT_SET_MULTIPLIER,DEFAULT_MULTIPLIER_CAP,STANDARD_SUITS,STANDARD_RANKS,SUIT_SYMBOLS,SHOWDOWN_PHASES,POKER_HANDS,POKER_STRENGTH,activeBattle,activeRun,numeric,signed,rankLabel,suitSymbol,unwrapCard,showdownValue,standardCardSpec,evaluatePoker,evaluateFourCardState,pokerStrength,handSummary,riverCandidateUniverse,createRiverCandidates,groupRiverCandidates,riverGroupText,riverSnapshotLines,riverHudText,createRiverSnapshot,captureRiverSnapshot,resolveRiverHit,bonusFromFactor,createMultiplierEntry,createBreakdown,addAdditive,addMultiplierBonus,addMultiplier,applyRiverHitBonus,detectPerfectSet,addPerfectSetMultiplier,finalizeSide,createAttack,compareFinalPower,finalizeBreakdown,percentText,multiplierText,additiveText,attackTraceText,comparisonTraceText,traceLines,snapshotBreakdown,cardLabel,advantageExtra,currentContractResolution,recordScoreTrigger,addActiveAdvantageMultipliers,animateBreakdown,clearShowdownSequence,resolveShowdownAttacks,archiveBreakdown,ensureRiverHud,syncRiverHud,resolveRuntimeShowdown,wrapPoker,wrapShowdown,wrapNextEnemy,wrapRenderBattle,installBrowser};
});
