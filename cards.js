...
const IMPLEMENTED_CARD_EFFECTS = {
  'pack01.black_bullet': [{trigger:'on_trick_win',action:'damage_enemy',value:3,duration:'trick'},{trigger:'on_showdown_score',action:'showdown_power',value:3,duration:'set'}],
  ...
  'pack01.sharp_glass': [{trigger:'on_trick_win',action:'apply_enemy_bleed',value:3,duration:'battle'}],
  ...
  'pack01.battery_1pct': [{trigger:'on_trick_end',handler:'deplete_battery_in_hand',chance:0.2,duration:'battle'},{trigger:'on_showdown_score',action:'showdown_power',value:12,condition:'slot_is',conditionValue:5,duration:'set'}]
};
const PLAYER_EFFECT_LABELS=Object.freeze({
  ...
  conditions:{chips_spent:'이번 트릭에 칩을 1 이상 소비',effective_rank_at_most:'트릭 숫자가 지정된 수 이하',effective_suit_is_trump:'최종 트릭 무늬가 현재 트럼프',river_hit:'4번째 트릭에 고정한 리버 후보를 5번째 카드로 적중',slot_is:'지정된 쇼다운 슬롯에 위치',slot_at_least:'지정된 쇼다운 슬롯 이후에 위치',pure_cards_at_least:'쇼다운 슬롯의 순수 카드가 지정 장수 이상',in_hand:'손패에 있음',player_has_advantage:'이번 쇼다운에 명시적 우세가 있음',enemy_has_advantage:'이번 쇼다운에 적의 명시적 우세가 있음',set_wins_at_least:'이번 세트의 트릭 승리가 지정 횟수 이상',pure_card_in_hand:'손패에 순수 카드가 1장 이상 있음',pure_card_in_showdown:'쇼다운 슬롯에 순수 카드가 1장 이상 있음',printed_equals_trick:'인쇄값과 트릭값이 같음',unmodified_trick_value:'다른 트릭값 보정이 없음'},
...
const CARD_DETAIL_BY_ID=Object.freeze({
  'pack01.black_bullet':{activation:'이 카드로 트릭 승리 시',effect:'적에게 피해 3.',extra:'이 카드가 쇼다운 5장에 포함되어 있으면 쇼다운 최종 위력 +3.',terms:['트릭','쇼다운','최종 위력']},
  ...
  'pack01.sharp_glass':{activation:'이 카드로 트릭 승리 시',effect:'적에게 출혈 3 부여.',extra:'출혈은 트릭 종료 시 피해를 주고 1 감소한다.',duration:'해당 전투가 끝나거나 출혈이 모두 감소할 때까지',terms:['트릭','출혈','피해']},
  ...
  'pack01.battery_1pct':{activation:'손에 들고 있는 동안 각 트릭 종료 시',condition:'20% 확률.',effect:'소진되어 이번 전투 동안 사용할 수 없다.',extra:'소진되지 않고 정확히 5번 쇼다운 슬롯에 제출하면 쇼다운 최종 위력 +12. 손에 없으면 소진을 판정하지 않는다.',duration:'현재 전투',terms:['트릭','소진','전투','쇼다운 슬롯','최종 위력']}
});
...
function browserFallbackMigratedDefinitions(){
  const rows=[
    {id:'core.paint',legacyTacticId:'paint',name:'트럼프 페인트',suit:'D',rank:4,text:'이 카드의 트릭 무늬를 현재 트럼프로 바꾸고 트릭 숫자 +1. 인쇄값과 쇼다운값은 변하지 않는다.',terms:['트릭값','트럼프','인쇄값','쇼다운값'],activation:'이 카드를 낼 때',migrationStage:'3-1',effects:[{trigger:'on_play',action:'set_next_trick_suit_to_trump',duration:'trick'},{trigger:'on_play',action:'increase_next_trick_rank',value:1,duration:'trick'}]},
    {id:'core.plus2',legacyTacticId:'plus2',name:'랭크 부스트',suit:'S',rank:3,text:'이 카드의 트릭 숫자 +2.',terms:['트릭값','인쇄값'],activation:'이 카드를 낼 때',migrationStage:'3-1',effects:[{trigger:'on_play',action:'increase_next_trick_rank',value:2,duration:'trick'}]},
    ...
    {id:'core.double',legacyTacticId:'double',name:'더블다운',suit:'H',rank:2,text:'이번 세트에서 트릭을 3번 이상 이겼다면 쇼다운 위력 +6.',terms:['쇼다운','트릭','최종 위력'],activation:'쇼다운 위력 계산 시',migrationStage:'7.5-P',effects:[{trigger:'on_showdown_score',action:'showdown_power',value:6,condition:'set_wins_at_least',conditionValue:3,duration:'set'}]},
    {id:'core.barrier',legacyTacticId:'barrier',name:'세이프가드',suit:'S',rank:6,text:'이 카드를 낼 때 보호막 3. 이 카드로 트릭 패배 시 보호막 3을 추가로 얻는다.',terms:['트릭','보호막'],activation:'이 카드를 낼 때',migrationStage:'3-1',effects:[{trigger:'on_play',action:'gain_shield',value:3,duration:'battle'},{trigger:'on_trick_loss',action:'gain_shield',value:3,duration:'battle'}]},
    {id:'core.burn',legacyTacticId:'burn',name:'패갈이',suit:'C',rank:2,text:'사용 시 손패에서 이 카드 외 1장을 선택해 버리고 칩 +1, 카드 1장을 뽑는다.',terms:['손패','버림','칩','드로우'],activation:'이 카드를 낼 때',migrationStage:'3-2B',targeting:{zone:'hand',count:1,excludeSelf:true},effects:[{trigger:'on_play',action:'discard_secondary_target',duration:'trick'},{trigger:'on_play',action:'gain_chips',value:1,duration:'trick'},{trigger:'on_play',action:'draw_cards',value:1,duration:'trick'}]},
    ...
    {id:'core.pureboost',legacyTacticId:'pureboost',name:'정공법',suit:'D',rank:5,text:'이 카드를 낼 때 이미 쇼다운 슬롯에 순수 카드가 1장 이상 있으면 트릭 숫자 +3.',terms:['순수 카드','쇼다운','트릭값'],activation:'이 카드를 낼 때',migrationStage:'7.5-P',effects:[{trigger:'on_play',action:'increase_next_trick_rank',value:3,condition:'pure_card_in_showdown',duration:'trick'}]},
    {id:'core.clean',legacyTacticId:'clean',name:'무첨가',suit:'S',rank:4,text:'이 카드로 트릭 승리 시 현재 쇼다운 슬롯에 순수 카드가 1장 이상 있으면 칩 +2.',terms:['순수 카드','쇼다운','트릭','칩'],activation:'이 카드로 트릭 승리 시',migrationStage:'7.5-P',effects:[{trigger:'on_trick_win',action:'gain_chips',value:2,condition:'pure_card_in_showdown',duration:'trick'}]},
    {id:'core.recolor',legacyTacticId:'recolor',name:'재도색',suit:'C',rank:9,text:'이 카드의 쇼다운 무늬를 현재 트럼프 무늬로 바꾼다.',terms:['쇼다운값','트럼프'],activation:'이 카드를 낼 때',migrationStage:'3-1',effects:[{trigger:'on_play',action:'set_last_showdown_suit_to_trump',duration:'set'}]},
