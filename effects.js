(function(root,factory){const api=factory();if(typeof module!=='undefined')module.exports=api;root.CardEffects=api;if(typeof document!=='undefined')api.loadLegacyTacticRuntime()})(typeof globalThis!=='undefined'?globalThis:this,function(){
  ...
  function run(trigger,card,context){if(!card?.named?.implemented)return 0;return runEffectList(card.named.effects.filter(x=>x.trigger===trigger),context)}
  ...
  return{TRIGGERS,ACTIONS,COPYABLE_NUMERIC_ACTIONS,conditions,handlers,runEffectList,run,resolveNextWinReservations,newHistory,loadLegacyTacticRuntime};
});
